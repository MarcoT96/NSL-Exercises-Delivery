...
/*
  ...
  the Algorithm (GA or SA) and represents the length of the particular path
...
class chromosome{
  ...
  public:
    ...
    */
    ...
    chromosome(std::vector <int>, std::vector <std::vector <double>>);  //non-trivial constructor
...
/*
  ...
  The Class annealer manages the Simulated Annealing Algorithm (SA).
  The methods defined below allow mutation operators to be applied
  to a single initial path (chromosome).
  Changes made by these genetic mutations are accepted or rejected
  through the Metropolis algorithm.
  The algorithm provides for the "slow cooling" of the path, 
  so among the data members there is also a schedule with respect 
  to the temperature.
...
class annealer{
  ...
  private:
    ...
    chromosome _p;  //The paths to be optimized
    std::map <double, unsigned int> _schedule;  //The annealing schedule {(ß_j, n_j)}
    double _acceptance;  //acceptance ratio in the Metropolis algorithm
  ...
  public:
    ...
    creation methods
    */
    annealer() {_p=chromosome(), _schedule={}, _acceptance=0;}  //trivial constructor
    annealer(chromosome, std::map <double, unsigned int>);  //non-trivial constructor
    annealer(chromosome, std::vector <double>, std::vector <unsigned int>);  //non-trivial constructor
    ~annealer() {}  //destructor
    /*
    set methods
    */
    void set_path(const chromosome& c) {_p=c;}
    void set_schedule(const std::map <double, unsigned int>& s) {_schedule=s;}
    void set_schedule(const std::vector <double>&, const std::vector <unsigned int>&);
    void set_acceptance(double acc) {_acceptance=acc;}
    /*
    get methods
    */
    chromosome get_path() const {return _p;}
    std::map <double, unsigned int> get_schedule() const {return _schedule;}
    double get_acceptance() const {return _acceptance;}
    double get_temperature(unsigned int) const;
    /*
    print methods
    */
    void print_path() const;
    void print_schedule() const;
    void check() const;  //check if all the bounds of the TSP are satisfied
    /*
    genetic-mutation operator
    */
    void pair_permutation();
    void pair_permutation(chromosome&);
    
    void contiguous_permutation();
    void contiguous_permutation(chromosome&);
    ...
    simulated annealing
    */
    void annealing(unsigned int, double, double, double);
};
/***********************************************************************************************/
/***********************************************************************************************/
/***********************************************************************************************/
